...
class ELOTrader(BaseTrader):
    ...
    required_order_dict_keys = ()

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.technology_subscription = Subscription('speed_tech', self.player_id, 
            kwargs['speed_unit_cost'])

    @property
    def delay(self):
        delay = self.default_delay
        if self.speed_subscriber is True:
            time_since_speed_change = time.time() - self.speed_change_time 
    ...
    def speed_change(self, **kwargs):
        self.__delay = kwargs['new_delay']
        self.speed_change_time = time.time()
        self.speed_subscriber = kwargs['value']
        self.event.broadcast_msgs(
            'speed_confirm', value=self.speed_subscriber, player_id=self.player_id, 
            market_id=self.market_id)

    def close_session(self, **kwargs):
        reference_price, tax_rate = kwargs['tax_rate'], kwargs['reference_price']  
        _, tax_paid = self.inventory.liquidify(reference_price, discount_rate=tax_rate)
        self.tax = tax_paid
        self.cash += self.inventory.cash
        self.cash -= self.speed_subscription_cost
    ...
    def executed(self, **kwargs):
        ...
        price = order_info['price']
        self.post_execution(
            price=price, buy_sell_indicator=buy_sell_indicator, **kwargs)   
        order_token = kwargs['order_token']
...
class ELOManualTrader(ELOTrader):
    ...
    def executed(self, **kwargs):
        # implement this via post execution
