...
def create(config, filename):   # TODO: Consider refactoring this 
    custom_config = dict()
    try:
        ...
        for k, v in config['files']['investors'].items():
            label = 'investors' + '_' + k
            csv_name = collecttool.extract_name(v)
            if csv_name not in all_files:
                h = collecttool.headers(my_token) 
                data = collecttool.get(v, h)
                collecttool.write(data, cfg_dir, csv_name)
