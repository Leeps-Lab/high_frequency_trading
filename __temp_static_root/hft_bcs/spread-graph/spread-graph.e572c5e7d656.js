    ...
    spreadGraph.bidArrow["bidArrowLine"] = spreadGraph.spread_svg.append("line")
        ...
        .call(d3.drag()
            ...
            .on("end", function(){
                    ...
                    var x = spreadGraph.askArrow["askArrowLine"].attr("x1");
                    var tickArray = spreadGraph.visibleTickLines.keys();
                    for(var i = 0; i < tickArray.length; i++){
                        if(spreadGraph.visibleTickLines[tickArray[i]] > x){
                            break;
                        }
                    }
                    var diffUpper = spreadGraph.visibleTickLines[tickArray[i]] - x;
                    var diffLower = spreadGraph.visibleTickLines[tickArray[i - 1]]  - x;
                    var snappedX = (diffUpper < diffUpper) ? spreadGraph.visibleTickLines[tickArray[i]] : spreadGraph.visibleTickLines[tickArray[i - 1]];
                    spreadGraph.bidArrow["bidArrowLine"].attr("x1",  snappedX).attr("x2", snappedX);
