  ...
  */
  ...
  drawArrows(){
    ...
    spreadGraph.bidArrow["bidArrowLine"] = spreadGraph.spread_svg.append("line")
        ...
        .attr("y2",spreadGraph.spread_height*0.6 + 10)  
        .attr("stroke","#B2D8B2")  
        .attr("stroke-width",5)  
        .attr("marker-end","url(#bidArrow)")
        .call(d3.drag()
        .on("drag", function(){
            d3.select(spreadGraph.bidArrow["bidArrowLine"]).attr("x1",  d3.event.x).attr("x2", d3.event.x);
        })
        //.on("drop", spreadGraph.dragMapToPrice)
    );
    ...
    spreadGraph.askArrow["askArrowLine"]  = spreadGraph.spread_svg.append("line")
        ...
        .call(d3.drag()
        .on("drag", function(){
            d3.select(spreadGraph.askArrow["askArrowLine"]).attr("x1",  d3.event.x).attr("x2", d3.event.x);
        })
        //.on("drop", spreadGraph.dragMapToPrice)
    );
