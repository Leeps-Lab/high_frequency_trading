  ...
  executionHandler(exec){
    ...
    if(otreeConstants.endMsg == "off"){
        ...
        if(exec.player == userPlayerID){
            ...
            spreadGraph.drawTransactionBar(exec_spread, svgMiddleY, otherYCoordinate, exec_side, ((exec.profit > 0) ? "transaction_bar_light_green" : "transaction_bar_light_red"), -10);
            if(exec_side == "B"){
                spreadGraph.spread_svg.selectAll(".other_line_bottom_"+exec.player).remove();
                var otherSpreadLineBottom = spreadGraph.spread_svg.append("svg:line")
                    .attr("x1", spreadGraph.spread_width)
                    .attr("y1", svgMiddleY + otherYCoordinate)
                    .attr("x2", spreadGraph.spread_width - 15)
                    .attr("y2", svgMiddleY + otherYCoordinate)
                    .attr("stroke-width",1)
                    .attr("class","other_line other_line_bottom_" + exec.player);
                
                spreadGraph.addOthersLineAnimation([otherSpreadLineBottom], 0, 15);
            } else if(exec_side == "S"){
                spreadGraph.spread_svg.selectAll(".other_line_top_" + exec.player).remove();       
                var otherSpreadLineTop = spreadGraph.spread_svg.append("svg:line")
                    .attr("x1", spreadGraph.spread_width)
                    .attr("y1",  svgMiddleY - otherYCoordinate)
                    .attr("x2", spreadGraph.spread_width - 15)
                    .attr("y2",  svgMiddleY - otherYCoordinate)
                    .attr("stroke-width",1)
                    .attr("class","other_line other_line_top_" + exec.player);
                spreadGraph.addOthersLineAnimation([otherSpreadLineTop], 0, 15);
            }
        }
    }
  }
            ...
            if(player == userPlayerID){
                ...
                if(role == "MAKER"){
                    ...
                    if(exec_side != "" && exec.player == userPlayerID){
                        if(exec.side == "B"){
                            spreadGraph.spread_svg.selectAll(".my_line_attempt").remove(); 
                            spreadGraph.addOthersLineAnimation([your_spread_line_bottom], transactionSpeed, 25);
                        } else if(exec.side == "S"){
                            spreadGraph.spread_svg.selectAll(".my_line_attempt").remove(); 
                            spreadGraph.addOthersLineAnimation([your_spread_line_top], transactionSpeed, 25); 
                        }
        ...
        for(var key in newLines){
            if(key == userPlayerID){
                ...
                if(role == "MAKER"){
                    ...
                    //     console.log(newLines[userPlayerID]);
                    // }
