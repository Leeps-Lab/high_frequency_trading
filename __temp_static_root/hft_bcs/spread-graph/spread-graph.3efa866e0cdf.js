...
// <svg id="timer"></svg>
  ...
  */
  ...
  BBOShift(shiftMsg){
    console.log("Shift received from BBO Msg Handler");
  ...
  animateBBOShift(desiredCenter){
    console.log("In Animate BBO Shift");
    ...
    spreadGraph.drawPossibleSpreadTicks(+newMiddlePrice - 50000, +newMiddlePrice + 50000);
  ...
  drawBidArrow(obj){
    ...
    spreadGraph.bidArrow["bidArrowText"].transition().duration(100).attr("x", spreadGraph.visibleTickLines[obj["price"]] - 10).on("end", function(){
  ...
  drawOfferArrow(obj){
    ...
    spreadGraph.askArrow["askArrowText"].transition().duration(100).attr("x", spreadGraph.visibleTickLines[obj["price"]] - 10).on("end", function(){
        spreadGraph.confirmArrow(obj);
    });;
