  ...
  */
  ...
  executionHandler(exec = {}){
    ...
    if(otreeConstants.endMsg == "off"){
        ...
        var role = document.querySelector('info-table').player_role;
        var sniper = false; 
        var currentFP =  (document.querySelector('info-table').fp)*10000;      
        ...
        if(exec.player == userPlayerID){
            if(spreadGraph.spread_lines[userPlayerID] != undefined){
                //you are a maker 
                var priceOfTransaction = spreadGraph.spread_lines[exec.player][exec.side.toString()];

                console.log("priceOfTransaction = " + priceOfTransaction);
                console.log("currentFP = " + currentFP);
