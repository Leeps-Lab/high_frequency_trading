...
.transaction_bar_dark_red{
 ...
 addOthersLineAnimation(lines, speed=500, width){
  ...
  drawSpreadBar(my_spread,svg_middle_y,y_coordinate, offset, id){
    ...
    updateBidAndAsk(FPCDollarAmmount){
        if(document.querySelector("info-table").player_role == "MAKER"){
            var spread_value = parseInt(document.querySelector('info-table').spread_value);
            document.querySelector('info-table').curr_bid = (FPCDollarAmmount + spread_value/2).toFixed(2);
            document.querySelector('info-table').curr_ask = (FPCDollarAmmount - spread_value/2).toFixed(2);
        }
    }
