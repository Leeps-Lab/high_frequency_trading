    ...
    Spread_Graph.spread_svg.on('click',function(d) { 
      var role = document.querySelector('info-table').player_role;
        if(role == "MAKER"){
          ...
          var distance_from_middle = Math.abs(clicked_point.y - fp_line_y - 42);
          ...
          Spread_Graph.sendSpreadChange(my_spread);
            ...
            //IF BUTTON IS NOT PRESSED OR ON THEN TURN IT ON AFTER DELAD (Button_Pressed())
                    ...
                    Input_Section.Button_Pressed("maker");

                    document.querySelector("input-section").shadowRoot.querySelector("#sniper").className = "button-out";
                    document.querySelector("input-section").shadowRoot.querySelector("#out").className = "button-out";
                    etElementById("spread-graph").getBoundingClientRect().top;

                    //Where the grey middle line is
                    svg_middle_x = Spread_Graph.spread_width / 2;
                    fp_line_y = Spread_Graph.spread_height / 2;
